...
class loginadmin:
    ...
    def __init__(self,root):
      ...
      img4=Image.open(r"images\ITM 1.jpg")
      ...
      letsstart=Label(frame,text=" Admin  Sign  In ",font=("times new roman",22,"bold"),fg="skyblue",bg="black")
    ...
    def resetpass(self):
      if self.securityname.get=="Select Questions":
        ...
        cur=conn.cursor()
        query=("select * from adminregister where username=%s and securityquestion=%s and securtiyanswer=%s")
        value=(self.txtuser.get(),self.securityname.get(),self.answername.get())
        cur.execute(query,value)
        row=cur.fetchone()
        if row==None:
          messagebox.showerror("error","please enter the correct answer",parent=self.root2)
        else:
          query=("update adminregister set password=%s where username=%s")
          value=(self.newpassname.get(),self.txtuser.get())
          cur.execute(query,value)
          conn.commit()
          conn.close()
    ...
    def forgotpass(self):
      if self.txtuser.get()=="":
        ...
        if row==None:
          ...
          self.securityname=ttk.Combobox(self.root2,font=("times new roman",17),width=16,state="readonly")
          ...
          self.securityname.place(x=260,y=60)

          answer=Label(self.root2,text="Security Answer      : ",font=("times new roman",18,"bold"),bg="black",fg="skyblue")
          answer.place(x=20,y=140)
          self.answername=ttk.Entry(self.root2,font=("times new roman",20))
          self.answername.place(x=260,y=140,width=200)

          

          newpass=Label(self.root2,text="New Password         : ",font=("times new roman",18,"bold"),bg="black",fg="skyblue")
          newpass.place(x=20,y=220)
          self.newpassname=ttk.Entry(self.root2,font=("times new roman",20))
          self.newpassname.place(x=260,y=220,width=200)
    ...
    def login(self):
      if self.txtuser.get()=="" or self.txtpassword.get()=="":
        ...
        if row==None:
          ...
          openmain=messagebox.askyesno("YesNo","Access Only Admin",parent=self.root)
          if openmain>0:
            self.newwindow=Toplevel(self.root)
            self.app=Student(self.newwindow)
          else:
